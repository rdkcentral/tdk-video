...
class HdmicecHalAgent : public RDKTestStubInterface , public AbstractServer<HdmicecHalAgent>
{
        public:
                ...
                HdmicecHalAgent(TcpSocketServer &ptrRpcServer) : AbstractServer <HdmicecHalAgent>(ptrRpcServer)
                {
					...
					this->bindAndAddMethod(Procedure("TestMgr_HdmicecHal_GetPhysicalAddress", PARAMS_BY_NAME, JSON_STRING, "handle", JSON_INTEGER, "Is_handle_invalid", JSON_INTEGER, "Is_null_param_check", JSON_INTEGER,  NULL), &HdmicecHalAgent::HdmicecHal_GetPhysicalAddress);
		...
		void HdmicecHal_AddLogicalAddress(IN const Json::Value& req, OUT Json::Value& response);
		void HdmicecHal_RemoveLogicalAddress(IN const Json::Value& req, OUT Json::Value& response);
